...
export type Probe={id:string;name:string;provider:string;endpoint_url:string;model:string;has_api_key:boolean;required_fields:string[];interval_minutes:number;timeout_ms:number;latency_slo_ms:number;availability_slo_percent:number;max_output_tokens:number;daily_token_cap:number;enabled:boolean;created_at:string;updated_at:string;last_observation:Observation|null;stats:Stats};
...
export type ProbeInput=Omit<Probe,'id'|'has_api_key'|'created_at'|'updated_at'|'last_observation'|'stats'> & {api_key:string;prompt:string};
